...
export default function HeroSection({ spotlights }: HeroSectionProps) {
  ...
  // Episode info (API sometimes returns counts as strings)
  ...
  const epInfo = currentAnime.tvInfo?.episodeInfo;
  const sub = epInfo
    ? parseNum(epInfo.sub)
    : parseNum(currentAnime.tvInfo?.sub);
  const dub = epInfo
    ? parseNum(epInfo.dub)
    : parseNum(currentAnime.tvInfo?.dub);
  const totalEps = parseNum(currentAnime.tvInfo?.eps) || sub + dub;
      ...
      <div className="relative z-10 container px-4 h-full flex items-center">
        <div className="max-w-2xl space-y-4">
          ...
          <div className="flex gap-3 mt-2">
            {firstEpisode && (
              <Link href={`/watch/${currentAnime.id}?ep=${firstEpisode.id}`}>
