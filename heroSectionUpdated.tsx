...
export default function HeroSection() {
  ...
  // Fetch home spotlights
  useEffect(() => {
    const fetchSpotlights = async () => {
      try {
        const data = await getHomeData(); // Fetch your spotlight anime
  ...
  if (loading) {
    return (
      <section className="relative w-full h-[60vh] md:h-[70vh] lg:h-[80vh] flex items-center justify-center bg-background">
        <p className="text-muted-foreground">Loading hero section...</p>
      </section>
    );
  }

  if (!hasSpotlights || !currentAnime) return null;

  // Episode info
  const hasEp = currentAnime.tvInfo?.episodeInfo;
  const sub = hasEp
    ? currentAnime.tvInfo!.episodeInfo!.sub
    : currentAnime.tvInfo?.sub || 0;
  const dub = hasEp
    ? currentAnime.tvInfo!.episodeInfo!.dub
    : currentAnime.tvInfo?.dub || 0;
    ...
    <section className="relative w-full h-[60vh] md:h-[70vh] lg:h-[80vh] overflow-hidden mb-10">
      ...
      <div className="relative z-10 container px-4 h-full flex items-center">
        <div className="max-w-2xl space-y-4">
          <h1 className="text-3xl md:text-5xl font-bold line-clamp-3">
            {currentAnime.title}
          </h1>
          ...
          {genres?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {genres.slice(0, 5).map((g: any) => (
                <span
                  key={g.name || g}
                  className="px-3 py-1 bg-muted/50 rounded-full border text-sm"
                >
                  {g.name || g}
                </span>
              ))}
          ...
          <div className="flex gap-3 mt-2">
            {firstEpisode && (
              <Link
                href={`/anime/${currentAnime.id}/episode/${firstEpisode.id}`}
              >
      ...
      <Button
        variant="ghost"
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-background/20"
        onClick={prevSlide}
      >
      ...
      <Button
        variant="ghost"
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-background/20"
        onClick={nextSlide}
      >
        ...
        {spotlights.map((_, i) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all ${
              i === currentSlide ? "bg-primary scale-125" : "bg-white/30"
            }`}
          />
        ))}
      </div>
