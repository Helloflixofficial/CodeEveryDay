...
const BASE_URL2 = 'https://gogoanimez.to/';
const ajax_url = 'https://www13.gogoanimes.fi/';
...
export const scrapeAnimeDetails = async ({ id }) => {
      try {
            ...
            const animePageTest = await axios.get(`https://gogoanimez.to/category/${id}`);
...
export const scrapeDownloadLinks = async ({ episodeId }) => {
      ...
      try {
            ...
            const e_page = await axios.get(`https://gogoanimez.to/${episodeId}`);
